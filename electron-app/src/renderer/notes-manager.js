...
class NotesManager {
  constructor() {
    ...
    this.editorMode = 'view'; // 'view' or 'edit'
  ...
  async openNote(noteName, heading = null, preserveScroll = false) {
    try {
      ...
      // Add to recent notes
      this.addToRecent(note);
      
      // If heading specified, scroll to it after a short delay to ensure rendering is complete
      if (heading) {
        setTimeout(() => this.scrollToHeading(heading), 100);
      }
  ...
  async renderMarkdown(markdown) {
    const viewDiv = document.getElementById('notes-editor-view');
    if (!viewDiv || !window.marked) return;
    
    let processed = markdown;
    
    // 1. Handle Obsidian image embeds: ![[image.png]] or ![[image.png|alt text]]
    processed = processed.replace(/!\[\[([^\]|]+\.(png|jpg|jpeg|gif|svg|webp))(?:\|([^\]]+))?\]\]/gi, (match, imagePath, ext, altText) => {
      const alt = altText || imagePath;
      const resolvedPath = this.resolveImagePath(imagePath);
      return `![${alt}](${resolvedPath})`;
    });
    
    // 2. Handle Obsidian callouts (before note embeds and wiki-links)
    processed = this.processCallouts(processed);
    
    // 3. Handle note embeds: ![[Note Name]] (non-image embeds)
    // Create placeholders and store embed data
    const embedData = await this.processNoteEmbeds(processed);
    processed = embedData.markdown;
    
    // 4. Convert Obsidian wiki-links [[Note Name]] to HTML links
    processed = processed.replace(/\[\[([^\]|#]+)(?:#([^\]|]+))?(?:\|([^\]]+))?\]\]/g, (match, target, heading, display) => {
      const linkText = display || (heading ? `${target}#${heading}` : target);
      const linkTarget = heading ? `${target}#${heading}` : target;
      return `<a href="#" data-wiki-link="${linkTarget}" class="wiki-link">${linkText}</a>`;
    });
    
    // Render markdown with custom renderer to add IDs to headings
    try {
      ...
      console.log('[Notes] Embed placeholders to replace:', Object.keys(embedData.embeds));
      
      // 5. Replace embed placeholders (HTML comments) with actual HTML embeds
      for (const [placeholder, embedHtml] of Object.entries(embedData.embeds)) {
        console.log(`[Notes] Replacing placeholder: "${placeholder}"`);
        
        if (html.includes(placeholder)) {
          html = html.replace(placeholder, embedHtml);
          console.log('[Notes] Successfully replaced placeholder');
        } else {
          console.warn(`[Notes] Placeholder not found in HTML: "${placeholder}"`);
          console.log('[Notes] HTML snippet:', html.substring(0, 500));
        }
      }
      
      viewDiv.innerHTML = html;
      
      // Setup click handlers for wiki-links
      this.setupWikiLinkHandlers(viewDiv);
    } catch (error) {
      console.error('[Notes] Error rendering markdown:', error);
      viewDiv.textContent = processed;
    }
  }
  
  /**
   * Process note embeds (![[Note Name]]) and replace with placeholders
   * Returns markdown with placeholders and a map of placeholder -> HTML
   * Prevents circular embeds by tracking depth
   */
  async processNoteEmbeds(markdown, depth = 0, visitedNotes = new Set()) {
    const MAX_EMBED_DEPTH = 3; // Prevent infinite recursion
    
    if (depth >= MAX_EMBED_DEPTH) {
      console.warn('[Notes] Max embed depth reached');
    ...
    while ((match = embedRegex.exec(markdown)) !== null) {
      const [fullMatch, noteName, section, displayText] = match;
      
      console.log(`[Notes] Found embed: "${noteName}"${section ? ' #' + section : ''}${displayText ? ' |' + displayText : ''}`);
      
      // Skip if it's an image (already handled)
      if (/\.(png|jpg|jpeg|gif|svg|webp)$/i.test(noteName)) {
        console.log(`[Notes] Skipping image embed: ${noteName}`);
        continue;
      }
    ...
    console.log(`[Notes] Processing ${embedList.length} note embeds at depth ${depth}`);
    
    // Storage for all embeds (including nested)
    const allEmbeds = {};
    ...
    for (let i = 0; i < embedList.length; i++) {
      ...
      const noteKey = `${embed.noteName}${embed.section ? '#' + embed.section : ''}`;
      
      if (visitedNotes.has(noteKey)) {
        console.warn(`[Notes] Circular embed detected: ${noteKey}`);
        const placeholder = `<!--EMBED-${Date.now()}-${i}-->`;
        const embedHtml = `<div class="embed-error">⚠️ Circular embed: ${embed.noteName}</div>`;
        allEmbeds[placeholder] = embedHtml;
        markdown = markdown.substring(0, embed.index) + placeholder + markdown.substring(embed.index + embed.fullMatch.length);
        continue;
      }
      
      // Fetch the embedded note
      const embeddedContent = await this.fetchEmbeddedNote(embed.noteName, embed.section);
      
      if (embeddedContent) {
        // Mark this note as visited
        const newVisited = new Set(visitedNotes);
        newVisited.add(noteKey);
        
        // Recursively process embeds in the embedded content
        const nestedResult = await this.processNoteEmbeds(embeddedContent, depth + 1, newVisited);
        
        // Merge nested embeds into our collection
        Object.assign(allEmbeds, nestedResult.embeds);
        
        // Render the embedded markdown to HTML
        const embeddedHtml = marked.parse(nestedResult.markdown);
        
        // Create placeholder and store the embed HTML
        const placeholder = `<!--EMBED-${Date.now()}-${i}-->`;
        const embedHtml = `<div class="note-embed" data-note="${embed.noteName}">
          <div class="note-embed-title">📄 ${embed.displayText || embed.noteName}</div>
          <div class="note-embed-content">${embeddedHtml}</div>
        </div>`;
        ...
        // Note not found
        const placeholder = `<!--EMBED-${Date.now()}-${i}-->`;
        const embedHtml = `<div class="embed-error">⚠️ Note not found: ${embed.noteName}</div>`;
        allEmbeds[placeholder] = embedHtml;
        markdown = markdown.substring(0, embed.index) + placeholder + markdown.substring(embed.index + embed.fullMatch.length);
      }
    }
    
    return { markdown, embeds: allEmbeds };
  }
  
  /**
   * Process Obsidian callouts
   * Converts callout syntax to custom HTML divs
   */
  processCallouts(markdown) {
    ...
    const calloutTypes = {
      ...
      hint: { icon: '💡', className: 'callout-hint' },
      important: { icon: '⚡', className: 'callout-important' },
      warning: { icon: '⚠️', className: 'callout-warning' },
      caution: { icon: '⚠️', className: 'callout-caution' },
      attention: { icon: '⚠️', className: 'callout-attention' },
      danger: { icon: '🔴', className: 'callout-danger' },
      error: { icon: '❌', className: 'callout-error' },
      failure: { icon: '❌', className: 'callout-failure' },
      success: { icon: '✅', className: 'callout-success' },
      check: { icon: '✅', className: 'callout-check' },
      done: { icon: '✅', className: 'callout-done' },
      question: { icon: '❓', className: 'callout-question' },
      help: { icon: '❓', className: 'callout-help' },
      faq: { icon: '❓', className: 'callout-faq' },
      bug: { icon: '🐛', className: 'callout-bug' },
      quote: { icon: '💬', className: 'callout-quote' },
      cite: { icon: '💬', className: 'callout-cite' }
    };
    ...
    return markdown.replace(calloutRegex, (match, type, title, content) => {
      ...
      const callout = calloutTypes[lowerType] || calloutTypes.note;
      ...
      // Title is optional - use type name if no title provided
      const displayTitle = title.trim() || type.charAt(0).toUpperCase() + type.slice(1);
      
      // Generate HTML placeholder (will be processed by markdown parser)
      const placeholder = `<!--CALLOUT-${Date.now()}-${Math.random()}-->`;
      
      // Store callout HTML (we'll use a simpler approach - convert to HTML-in-markdown)
      return `<div class="callout ${callout.className}">
<div class="callout-title">${callout.icon} ${displayTitle}</div>
...
${processedContent}
  ...
  /**
   * Scroll to a heading in the current note
   */
  scrollToHeading(heading) {
    const headingId = this.slugifyHeading(heading);
    const viewDiv = document.getElementById('notes-editor-view');
    
    if (!viewDiv) {
      console.warn('[Notes] Editor view not found');
      return;
    }
    
    const element = viewDiv.querySelector(`#${headingId}`);
    
    if (element) {
      console.log(`[Notes] Scrolling to heading: ${heading} (id: ${headingId})`);
      ...
      console.warn(`[Notes] Heading not found: ${heading} (id: ${headingId})`);
    }
  }
  
  /**
   * Setup click handlers for wiki-links in rendered markdown
   */
  setupWikiLinkHandlers(container) {
    const links = container.querySelectorAll('a[data-wiki-link]');
    ...
    links.forEach(link => {
      link.addEventListener('click', async (e) => {
        ...
        if (target.includes('#')) {
          ...
          await this.openNote(noteName, heading);
        } else {
  ...
  enterEditMode(cursorPos = null) {
    ...
    if (cursorPos !== null && cursorPos >= 0) {
      ...
      console.log('[Notes] Entered edit mode at position:', cursorPos);
    } else {
      console.log('[Notes] Entered edit mode');
    }
  }
   ...
   */
  async exitEditMode() {
    console.log('[Notes] exitEditMode called, stack:', new Error().stack);
    ...
    if (this.hasUnsavedChanges) {
      // Cancel auto-save timeout since we're saving immediately
      if (this.saveTimeout) {
    ...
    console.log('[Notes] Exited to view mode');
  }

  /**
   * Calculate cursor position in markdown source from click on rendered HTML
   */
  calculateCursorPositionFromClick(event, viewDiv) {
    try {
      // Get the text node and offset at click position
      const range = document.caretRangeFromPoint(event.clientX, event.clientY);
      if (!range) {
        console.log('[Notes] Could not get range from click position');
        return null;
      }
      ...
      // Get all text content up to the click position
      let textBeforeClick = '';
      let foundClick = false;
      
      // Walk through all text nodes in the view div
      ...
      while (node = walker.nextNode()) {
        if (node === range.startContainer) {
          // This is the clicked text node
          textBeforeClick += node.textContent.substring(0, range.startOffset);
          foundClick = true;
          break;
        } else {
          // Add all text from nodes before the clicked one
          textBeforeClick += node.textContent;
        }
      }
      
      if (!foundClick) {
        console.log('[Notes] Could not find clicked node in tree');
        return null;
      }
      ...
      // Try to find the position in markdown
      // This is approximate because rendered HTML !== markdown source
      // We'll search for the text content in the markdown
      
      if (textBeforeClick.length === 0) {
        console.log('[Notes] Click at start of document');
        return 0;
      }
      
      // Remove extra whitespace and normalize
      const normalizedText = textBeforeClick.trim();
      
      // Try to find this text in the markdown
      // Search for the last occurrence of a reasonable chunk of text
      const searchText = normalizedText.slice(-50); // Last 50 chars
      const position = markdown.lastIndexOf(searchText);
      
      if (position >= 0) {
        // Found it! Position cursor at the end of this text
        const cursorPos = position + searchText.length;
        console.log('[Notes] Calculated cursor position:', cursorPos, 'from text:', searchText.slice(-20));
        return cursorPos;
      }
      
      // Fallback: try to estimate based on text length ratio
      const renderedTextLength = viewDiv.textContent.length;
      const clickTextLength = textBeforeClick.length;
      const ratio = clickTextLength / renderedTextLength;
      const estimatedPos = Math.floor(markdown.length * ratio);
      
      console.log('[Notes] Estimated cursor position:', estimatedPos, 'using ratio:', ratio.toFixed(2));
      return estimatedPos;
      
    } catch (error) {
      console.error('[Notes] Error calculating cursor position:', error);
      return null;
    }
  }
  ...
  /**
   * Setup view toggle functionality for right sidebar
   */
  setupSidebarViewToggle() {
    const toggleBtns = document.querySelectorAll('.sidebar-view-btn');
   ...
   */
  async showNoteSuggestions(textarea, cursorPos) {
    console.log('[Notes] showNoteSuggestions called, cursorPos:', cursorPos);
    
    // Create dropdown if it doesn't exist
    let dropdown = document.getElementById('note-suggestions-dropdown');
    if (!dropdown) {
      console.log('[Notes] Creating suggestions dropdown');
      dropdown = document.createElement('div');
      dropdown.id = 'note-suggestions-dropdown';
      ...
      console.log('[Notes] Dropdown appended to body');
    }
    
    // Position dropdown near textarea
    const rect = textarea.getBoundingClientRect();
    
    // Position below the textarea, slightly to the right
    let top = rect.top + 60; // Below the top of textarea
    let left = rect.left + 20; // Slightly indented
    ...
    console.log('[Notes] Dropdown positioned at:', { left: `${left}px`, top: `${top}px` });
    
    // Load and show suggestions
    await this.updateNoteSuggestions('');
    
    console.log('[Notes] Dropdown shown');
  }
  
  /**
   * Update note suggestions based on query
   */
  async updateNoteSuggestions(query) {
    console.log('[Notes] updateNoteSuggestions called with query:', query);
    const dropdown = document.getElementById('note-suggestions-dropdown');
    if (!dropdown) {
      console.error('[Notes] Dropdown not found in updateNoteSuggestions');
      return;
    }
    
    try {
      // Fetch notes from backend
      console.log('[Notes] Fetching notes from backend...');
      const response = await fetch(`http://127.0.0.1:11436/polly/notes/index?limit=50`);
      if (!response.ok) {
        console.error('[Notes] Failed to fetch notes for suggestions');
        return;
      }
      ...
      if (query) {
        ...
        console.log('[Notes] Filtered to', notes.length, 'notes matching:', query);
      }
      ...
      // Render suggestions
      if (notes.length === 0) {
        dropdown.innerHTML = '<div style="padding: 12px; color: var(--text-secondary); font-size: 0.9em;">No notes found</div>';
      } else {
        console.log('[Notes] Rendering', notes.length, 'suggestions');
        dropdown.innerHTML = notes.map((note, index) => `
          <div class="suggestion-item ${index === 0 ? 'selected' : ''}" data-note-name="${note.name}" style="
            padding: 8px 12px;
            cursor: pointer;
            border-bottom: 1px solid var(--border-color);
            transition: background 0.2s;
          ">
            <div style="font-weight: 500; color: var(--text-primary);">${note.name}</div>
            ${note.folder ? `<div style="font-size: 0.85em; color: var(--text-secondary); margin-top: 2px;">${note.folder}</div>` : ''}
          </div>
        `).join('');
        
        // Add mousedown handlers (fires before blur, unlike click)
        dropdown.querySelectorAll('.suggestion-item').forEach(item => {
          item.addEventListener('mousedown', (e) => {
            ...
            console.log('[Notes] Suggestion clicked:', noteName);
            console.log('[Notes] Editor mode:', this.editorMode);
            const textarea = document.getElementById('notes-editor-textarea');
            console.log('[Notes] Textarea exists:', !!textarea);
            if (textarea) {
              console.log('[Notes] Textarea cursor pos:', textarea.selectionStart);
            }
            
            // Insert the note name
            this.insertNoteName(noteName);
          ...
          item.addEventListener('mouseenter', () => {
            dropdown.querySelectorAll('.suggestion-item').forEach(i => i.classList.remove('selected'));
            item.classList.add('selected');
          });
        });
      }
      ...
      console.log('[Notes] Suggestions rendered successfully');
      
    } catch (error) {
      console.error('[Notes] Error updating suggestions:', error);
    }
  }
  ...
  insertSelectedSuggestion(textarea) {
    ...
    const noteName = selected.getAttribute('data-note-name');
    this.insertNoteName(noteName);
  ...
  /**
   * Insert note name at cursor position
   */
  insertNoteName(noteName) {
    console.log('[Notes] insertNoteName called with:', noteName);
    
    // Make sure we're in edit mode
    if (this.editorMode !== 'edit') {
      console.error('[Notes] Not in edit mode, cannot insert');
      return;
    }
    
    const textarea = document.getElementById('notes-editor-textarea');
    if (!textarea) {
      console.error('[Notes] Textarea not found, editorMode:', this.editorMode);
      return;
    }
    
    const text = textarea.value;
    const cursorPos = textarea.selectionStart;
    console.log('[Notes] Current cursor position:', cursorPos);
    
    // Find the [[ before cursor and ]] after cursor
    const beforeCursor = text.substring(0, cursorPos);
    const afterCursor = text.substring(cursorPos);
    
    console.log('[Notes] Before cursor:', beforeCursor.slice(-20));
    console.log('[Notes] After cursor:', afterCursor.slice(0, 20));
    
    const matchBefore = beforeCursor.match(/(!?\[\[)([^\]]*)$/);
    const matchAfter = afterCursor.match(/^([^\]]*)(\]\])/);
    
    console.log('[Notes] Match before:', matchBefore);
    console.log('[Notes] Match after:', matchAfter);
    
    if (matchBefore && matchAfter) {
      const linkStart = beforeCursor.lastIndexOf(matchBefore[0]);
      const contentAfterBrackets = matchAfter[1].length;
      
      // Replace everything between [[ and ]]
      const before = text.substring(0, linkStart + matchBefore[1].length);
      const after = text.substring(cursorPos + contentAfterBrackets + 2); // +2 for ]]
      
      console.log('[Notes] Replacing:', matchBefore[2], 'with:', noteName);
      
      textarea.value = before + noteName + ']]' + after;
      const newPos = before.length + noteName.length;
      textarea.selectionStart = newPos;
      textarea.selectionEnd = newPos;
      
      console.log('[Notes] Inserted note name, new cursor pos:', newPos);
      
      this.hideNoteSuggestions();
      
      // Don't call onEditorChange immediately - it triggers auto-save which causes reload
      // Instead, just mark as changed
      if (this.saveTimeout) {
        clearTimeout(this.saveTimeout);
      }
      this.saveTimeout = setTimeout(() => {
      ...
      console.error('[Notes] Could not find [[ ]] around cursor');
    }
  }
  
  /**
   * Hide note suggestions dropdown
   */
  hideNoteSuggestions() {
    const dropdown = document.getElementById('note-suggestions-dropdown');
    if (dropdown) {
      ...
      console.log('[Notes] Dropdown hidden');
    }
  }
}
