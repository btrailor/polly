...
const PollyBridge = {
  ...
  async safeCall(method, ...args) {
    ...
    if (!ready) {
      console.error(`[PollyBridge] Cannot call ${method}: IPC bridge not available`);
      return null;
    }
    
    try {
      if (typeof window.polly[method] !== 'function') {
        console.error(`[PollyBridge] Method ${method} not found on window.polly`);
        return null;
      }
   ...
   */
  isReady() {
    return this._ready || (typeof window.polly !== 'undefined' && window.polly !== null);
  }
};
...
async function checkPollyInitStatus() {
  try {
    ...
    console.debug('[Init] Polly status check error (expected during startup):', error.message);
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (queryInput) {
    ...
    console.log('query-input parent display:', window.getComputedStyle(queryInput.parentElement).display);
  }
...
const initialize = withErrorBoundary(async function() {
  ...
  console.log('[Init DEBUG] center-content exists:', !!centerContent, 'visible:', centerContent ? window.getComputedStyle(centerContent).display : 'N/A');
  console.log('[Init DEBUG] right-sidebar exists:', !!rightSidebar, 'visible:', rightSidebar ? window.getComputedStyle(rightSidebar).display : 'N/A');
  console.log('[Init DEBUG] left-sidebar exists:', !!leftSidebar, 'visible:', leftSidebar ? window.getComputedStyle(leftSidebar).display : 'N/A');
  
  const bridgeReady = await PollyBridge.waitForReady();
  
  if (!bridgeReady) {
    showToast('Failed to initialize Polly IPC bridge. Please restart the app.', 'error');
    return;
  }
  ...
  const savedCodePaths = await PollyBridge.safeCall('getStore', 'codebasePaths');
  ...
  if (setupComplete) {
    ...
    try {
      ...
      console.error('[Init] Failed to initialize conversations, but continuing:', error);
...
async function initializeConversations() {
  try {
    ...
    if (preservedUIState.conversationId) {
      const exists = conversations.find(c => c.id === preservedUIState.conversationId);
      if (exists) {
        targetConversationId = preservedUIState.conversationId;
        console.log('[Init] Restoring conversation from preserved state:', targetConversationId);
      }
    }
...
async function switchToConversation(conversationId, options = {}) {
  try {
    ...
    if (!isSameConversation || !hasMessagesInUI) {
      ...
      console.log('[Switch] Same conversation reload, skipping clear/re-render');
    }
    ...
    if (restoreScroll && preservedUIState.scrollPosition > 0) {
      setTimeout(() => {
        if (container) {
          ...
          console.log('[Switch] Restored scroll position:', preservedUIState.scrollPosition);
        }
  ...
  tabButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (targetTab === 'api-keys') {
        ...
        console.log('[Settings Tab] loadBudgetStatus type:', typeof loadBudgetStatus);
...
function setupEventListeners() {
  ...
  function updateTitlebarButtons() {
    const leftCollapsed = threeColumnLayout?.classList.contains('left-collapsed');
    const rightCollapsed = threeColumnLayout?.classList.contains('right-collapsed');
    ...
    if (titlebarToggleLeft) {
      ...
      if (icon) {
        icon.setAttribute('data-lucide', leftCollapsed ? 'chevron-right' : 'chevron-left');
      }
      ...
      titlebarToggleLeft.title = leftCollapsed ? 'Show left sidebar (Cmd+B)' : 'Hide left sidebar (Cmd+B)';
    }
    ...
    if (titlebarToggleRight) {
      ...
      if (icon) {
        icon.setAttribute('data-lucide', rightCollapsed ? 'chevron-left' : 'chevron-right');
      }
      ...
      titlebarToggleRight.title = rightCollapsed ? 'Show right sidebar (Cmd+/)' : 'Hide right sidebar (Cmd+/)';
    }
  ...
  function toggleLeftSidebar() {
    if (threeColumnLayout) {
      const isCollapsed = threeColumnLayout.classList.contains('left-collapsed');
  ...
  function toggleRightSidebar() {
    if (threeColumnLayout) {
      const isCollapsed = threeColumnLayout.classList.contains('right-collapsed');
  ...
  if (conversationsToggle) {
    conversationsToggle.addEventListener('click', () => {
      ...
      if (section) {
        section.style.display = section.style.display === 'none' ? 'block' : 'none';
        if (icon) {
          icon.setAttribute('data-lucide', section.style.display === 'none' ? 'chevron-right' : 'chevron-down');
  ...
  if (searchInput) {
    ...
    console.warn('Conversations search input not found (may not be on chat view)');
  }
  ...
  const regenerateCurriculumBtn = document.getElementById('btn-regenerate-curriculum');
  ...
  const confirmCompleteSectionBtn = document.getElementById('btn-confirm-complete-section');
  if (confirmCompleteSectionBtn) {
    confirmCompleteSectionBtn.addEventListener('click', handleConfirmCompleteSection);
  }
}
...
function updateLeftSidebar(view) {
  ...
  const sidebarConfigs = {
    ...
    calendar: {
      title: 'Timeline',
      content: '<div style="padding: 16px; color: #808080; font-size: 13px;">Calendar navigation coming soon</div>'
    },
    mail: {
      title: 'Folders',
      content: '<div style="padding: 16px; color: #808080; font-size: 13px;">Mail folders coming soon</div>'
    },
    code: {
      title: 'Files',
      content: '<div style="padding: 16px; color: #808080; font-size: 13px;">File browser coming soon</div>'
    },
    projects: {
      title: 'Projects',
      content: '<div style="padding: 16px; color: #808080; font-size: 13px;">Project list coming soon</div>'
    },
    notes: {
      title: 'Notes',
      content: `
    ...
    curricula: {
      title: 'My Curricula',
      content: '<div style="padding: 16px; color: #808080; font-size: 13px;">Use the Professor persona to create curricula</div>'
    },
    search: {
      title: 'Recent',
      content: '<div style="padding: 16px; color: #808080; font-size: 13px;">Recent searches coming soon</div>'
    },
    domains: {
      title: 'Domains',
      content: '<div style="padding: 16px; color: #808080; font-size: 13px;">Domain list coming soon</div>'
    }
  };
  ...
  setTimeout(() => {
    if (view === 'dashboard') {
      ...
      document.querySelectorAll('.left-sidebar .nav-item[data-view]').forEach(btn => {
        btn.addEventListener('click', () => {
      ...
      const indexObsidianBtn = document.getElementById('sidebar-index-obsidian');
      ...
      const categorySelect = document.getElementById('sidebar-pattern-category');
      ...
      document.querySelectorAll('.settings-nav-item').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          if (tab === 'domains') {
            ...
            console.log('[Settings Sidebar] loadAPIKeys type:', typeof loadAPIKeys);
            console.log('[Settings Sidebar] loadBudgetStatus type:', typeof loadBudgetStatus);
            if (typeof loadAPIKeys === 'function') {
...
function renderDashboardSidebar() {
  ...
  const recentConvsHTML = recentConvs.length > 0 ? `
...
function reattachChatEventListeners() {
  ...
  if (conversationsToggle) {
    conversationsToggle.addEventListener('click', () => {
      ...
      if (section) {
        ...
        if (icon) {
          icon.setAttribute('data-lucide', section.classList.contains('collapsed') ? 'chevron-right' : 'chevron-down');
  ...
  const floatingPersonaSelect = document.getElementById('floating-persona-select');
  ...
  if (orchestratorToggle) {
    orchestratorToggle.addEventListener('click', () => {
      ...
      console.log('[Orchestrator] Toggled:', !isActive, '(not yet implemented)');
    });
  }
...
function updateRightSidebar(view) {
  ...
  if (originalChatHTML === null && sidebarContent.querySelector('#query-input')) {
...
function renderDashboardRightSidebar() {
  ...
  const todayConvs = conversations.filter(c => {
    const createdToday = new Date(c.created_at).toDateString() === new Date().toDateString();
    return createdToday;
  }).length;
  
  // Get most recent conversation
  const recentConv = conversations.sort((a, b) => 
    new Date(b.updated_at || b.created_at) - new Date(a.updated_at || a.created_at)
  )[0];
  
  const lastActivity = recentConv ? formatRelativeTime(recentConv.updated_at || recentConv.created_at) : 'No activity';
...
function renderKnowledgeRightSidebar() {
  ...
  const notesStats = window.pollyStats?.rag_stats?.notes || window.pollyStats?.rag_stats?.obsidian || { count: 0, files: 0 };
...
async function checkDependencies() {
  ...
  if (deps.python) {
    pythonEl.querySelector('.dep-icon').innerHTML = '<i data-lucide="check-circle" style="width: 16px; height: 16px; color: var(--success);"></i>';
    pythonEl.querySelector('.dep-status').textContent = 'Installed';
  } else {
    pythonEl.querySelector('.dep-icon').innerHTML = '<i data-lucide="x-circle" style="width: 16px; height: 16px; color: var(--error);"></i>';
  ...
  if (deps.ollama) {
    ollamaEl.querySelector('.dep-icon').innerHTML = '<i data-lucide="check-circle" style="width: 16px; height: 16px; color: var(--success);"></i>';
    ollamaEl.querySelector('.dep-status').textContent = 'Installed';
  } else {
    ollamaEl.querySelector('.dep-icon').innerHTML = '<i data-lucide="alert-circle" style="width: 16px; height: 16px; color: var(--warning);"></i>';
...
async function handlePersonaChange(e) {
  ...
  try {
    ...
    console.log('[Persona] Response status:', response.status, response.statusText);
    ...
    if (data.success && data.state) {
      ...
      if (persona && persona.available_modes) {
        ...
        console.log('[Persona] floatingModeSelect before:', floatingModeSelect?.innerHTML);
        
        const modeOptionsHTML = persona.available_modes.map(mode => 
        ...
        if (floatingModeSelect) {
          ...
          console.log('[Persona] floatingModeSelect after:', floatingModeSelect.innerHTML);
          console.log('[Persona] floatingModeSelect.disabled:', floatingModeSelect.disabled);
        }
        
        console.log(`[Persona] Activated ${personaName} in ${data.state.current_mode} mode`);
    ...
    if (floatingPersonaSelect && floatingPersonaSelect !== e.target) floatingPersonaSelect.value = '';
...
function updateTeachingModeIndicator(persona, mode) {
  ...
  if (persona === 'professor') {
    ...
    indicator.classList.remove('mode-socratic', 'mode-explain', 'mode-curriculum', 'mode-quiz');
...
async function restorePersonaState() {
  try {
    ...
    if (data.active_persona) {
      ...
      const persona = listData.personas.find(p => p.name === data.active_persona);
      
      if (persona && persona.available_modes) {
        // Populate mode selector
        modeSelect.disabled = false;
        modeSelect.innerHTML = persona.available_modes.map(mode => 
          `<option value="${mode}" ${mode === data.current_mode ? 'selected' : ''}>${mode.charAt(0).toUpperCase() + mode.slice(1)}</option>`
        ).join('');
        
        console.log(`[Persona] Restored ${data.active_persona} in ${data.current_mode} mode`);
...
async function sendToPersona(query, conversationHistory, loadingId) {
  try {
    ...
    console.log('[Persona] Response received with', personaResponse.actions?.length || 0, 'actions');
...
async function handlePersonaAction(action) {
  ...
  switch (action.type) {
    ...
    case 'show_template_gallery':
      ...
      console.log('[Persona] window.TemplateGallery exists:', !!window.TemplateGallery);
      ...
      if (window.TemplateGallery && action.data) {
        ...
        console.log('[Persona] Showing template gallery, suggested:', suggestedTemplate);
    ...
    case 'auto_mode_switch':
      ...
      if (action.data && action.data.target_mode) {
        ...
        if (modeSelect) {
          ...
          const existingOption = Array.from(modeSelect.options).find(opt => opt.value === targetMode);
          if (!existingOption) {
            // Add the mode option if it doesn't exist
            const option = document.createElement('option');
            option.value = targetMode;
            option.textContent = targetMode.charAt(0).toUpperCase() + targetMode.slice(1);
          ...
          setTimeout(async () => {
            console.log('[Persona] Auto-triggering next step in', targetMode, 'mode');
            ...
            // Show loading
            const followUpLoadingId = addMessageToUI('assistant', '<div class="loading"><div class="loading-spinner"></div>Processing...</div>');
            
            // Send continuation query
            await sendToPersona('Continue workflow', conversationHistory, followUpLoadingId);
          }, 500);
        }
    ...
    case 'suggest_mode_switch':
      ...
      if (action.data && action.data.target_mode) {
        const targetMode = action.data.target_mode;
        const message = action.data.message || `Consider switching to ${targetMode} mode`;
        console.log('[Persona] Mode switch suggested:', targetMode, '-', message);
...
async function sendAnswersToPersona(answers) {
  ...
  // Show loading
  const loadingId = addMessageToUI('assistant', '<div class="loading"><div class="loading-spinner"></div>Generating note...</div>');
...
async function sendTemplateSelectionToPersona(templateFilename) {
  ...
  // Show loading
  const loadingId = addMessageToUI('assistant', '<div class="loading"><div class="loading-spinner"></div>Generating note from template...</div>');
  
  // Switch to enrich mode
  try {
    console.log('[Persona] Switching to enrich mode...');
    const modeResponse = await fetch('http://127.0.0.1:11436/persona/switch-mode', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({mode: 'enrich'})
    });
    ...
    console.error('[Persona] Failed to switch mode:', error);
    removeMessage(loadingId);
    addMessageToUI('assistant', `Error switching to enrich mode: ${error.message}`);
    return;
  }
  ...
  try {
    console.log('[Persona] Sending process request with template:', templateFilename);
    ...
    console.log('[Persona] Process response status:', response.status, response.statusText);
    ...
    const personaResponse = data.response;
    console.log('[Persona] Response actions:', personaResponse.actions?.map(a => a.type));
    ...
    // Handle any actions
    if (personaResponse.actions && personaResponse.actions.length > 0) {
      console.log('[Persona] Processing', personaResponse.actions.length, 'action(s)');
      for (const action of personaResponse.actions) {
...
async function saveNoteToKnowledgeBase(noteData) {
  try {
    ...
    if (!content) {
      console.error('[Persona] Note data structure:', JSON.stringify(noteData, null, 2));
    ...
    if (result.status === 'similar_found') {
      ...
      const message = `Found ${result.similar_notes.length} similar note(s):\n` +
        result.similar_notes.map(n => `- ${n.title} (${Math.round(n.similarity * 100)}% similar)`).join('\n') +
      ...
      const createResponse = await fetch('http://127.0.0.1:11436/polly/notes/create', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
          name: title,
...
async function getCurriculum(curriculumId) {
  try {
    const result = await safeFetch(`${API_URL}/polly/curricula/${curriculumId}`);
    console.log('[getCurriculum] safeFetch result:', { ok: result.ok, hasData: !!result.data, dataType: typeof result.data });
    ...
    console.log('[getCurriculum] result.data keys:', Object.keys(result.data || {}));
...
async function activateCurriculum(curriculumId) {
  try {
    const response = await fetch(`${API_URL}/polly/curricula/${curriculumId}/activate`, {
      method: 'POST'
    });
...
async function pauseCurriculum(curriculumId) {
  try {
    const response = await fetch(`${API_URL}/polly/curricula/${curriculumId}/pause`, {
      method: 'POST'
    });
...
async function completeCurriculum(curriculumId) {
  try {
    const response = await fetch(`${API_URL}/polly/curricula/${curriculumId}/complete`, {
      method: 'POST'
    });
...
async function getCurriculumProgress(curriculumId) {
  try {
    const result = await safeFetch(`${API_URL}/polly/curricula/${curriculumId}/progress`);
    if (!result.ok) {
...
async function enrichSection(curriculumId, sectionId) {
  try {
    console.log('[Enrich Section] Starting enrichment for', curriculumId, sectionId);
...
async function endLearningSession(sessionId, summary = {}) {
  try {
    const response = await safeFetch(
      `${API_URL}/polly/curricula/session/end`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function getCurriculumTemplate(templateId) {
  try {
    const response = await safeFetch(`${API_URL}/polly/curricula/templates/${templateId}`);
    return await response.json();
  } catch (error) {
...
async function loadLearningSidebarCurricula() {
  ...
  try {
    ...
    container.innerHTML = curricula.map(c => `
    ...
    container.querySelectorAll('.btn-view-curriculum, .curriculum-sidebar-item').forEach(el => {
...
async function detectCurriculumTemplate(goal) {
  try {
    const result = await safeFetch(`${API_URL}/polly/curricula/templates/detect`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function loadCurriculaView() {
  ...
  // Show loading state
  container.innerHTML = '<div class="loading-spinner">Loading curricula...</div>';
  ...
  try {
    ...
    container.innerHTML = curricula.map(c => renderCurriculumCard(c)).join('');
    lucide.createIcons();
...
async function loadCurriculumDetail(curriculumId) {
  ...
  try {
    ...
    console.log('[Curriculum Detail] Sections type:', typeof curriculum.sections, 'Is array:', Array.isArray(curriculum.sections));
    console.log('[Curriculum Detail] Sections length:', curriculum.sections?.length);
    ...
    document.getElementById('btn-close-curriculum')?.addEventListener('click', () => {
...
function renderCurriculumSections(curriculum) {
  ...
  console.log('[Render Sections] Curriculum is undefined?', curriculum === undefined);
  console.log('[Render Sections] Curriculum keys:', curriculum ? Object.keys(curriculum) : 'N/A');
  ...
  if (!Array.isArray(sections)) {
    console.error('[Render Sections] Sections is not an array:', typeof sections, sections);
  ...
  const subSections = sections.filter(s => s.type === 'subheading' || s.parent_id);
  ...
  weekSections.forEach(week => {
    ...
    const weekCompleted = children.filter(s => s.status === 'completed').length;
  ...
  const orphanSections = sections.filter(s => s.type !== 'week' && !s.parent_id);
  if (orphanSections.length > 0) {
    html += orphanSections.map(s => renderCurriculumSection(s, curriculum.id)).join('');
  }
  
  return html || '<div style="color: #808080; font-size: 13px; padding: 20px; text-align: center;">No sections defined</div>';
}
...
function renderCurriculumSection(section, curriculumId) {
  const statusIcon = section.status === 'completed' ? 'check-circle' : 
                     section.status === 'in_progress' ? 'circle-dot' : 'circle';
  const statusColor = section.status === 'completed' ? '#8fd98f' : 
                      section.status === 'in_progress' ? '#f0903b' : '#606060';
...
function attachCurriculumSectionHandlers(curriculum) {
  ...
  document.querySelectorAll('.btn-complete-section').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      showCompleteSectionDialog(curriculumId, sectionId, section?.title || 'Section');
    });
  });
}
...
async function handleViewSection(curriculumId, sectionId) {
  try {
    console.log('[View Section] Loading materials for:', curriculumId, sectionId);
 ...
 */
function displaySectionMaterials(curriculumId, sectionId, enrichment, templateId) {
  console.log('[Display Materials] Showing enrichment for:', sectionId, enrichment);
  console.log('[Display Materials] Enrichment keys:', Object.keys(enrichment || {}));
  console.log('[Display Materials] Template ID:', templateId);
  
  // Determine if this is a programming curriculum
  const isProgramming = templateId && ['programming-language', 'framework-library'].includes(templateId);
  const examplesHeading = isProgramming ? 'Code Examples' : 'Real-World Examples';
    ...
    <div style="padding: 20px; max-width: 900px; margin: 0 auto;">
      ...
      ${enrichment.diagrams && enrichment.diagrams.length > 0 ? `
        <div style="margin-bottom: 32px;">
          ...
          ${enrichment.diagrams.map((diagram, idx) => `
      ...
      ${enrichment.examples && enrichment.examples.length > 0 ? `
        <div style="margin-bottom: 32px;">
          ...
          ${enrichment.examples.map((example, idx) => `
            <div style="background: #252525; padding: 16px; border-radius: 8px; margin-bottom: 16px;">
      ...
      ${enrichment.exercises && enrichment.exercises.length > 0 ? `
        <div style="margin-bottom: 32px;">
          ...
          ${enrichment.exercises.map((exercise, idx) => {
      ...
      ${enrichment.resources && enrichment.resources.length > 0 ? `
        <div style="margin-bottom: 32px;">
          ...
          <div style="display: flex; flex-direction: column; gap: 8px;">
            ${enrichment.resources.map(resource => `
      ...
      <!-- Assessment Criteria -->
      ${enrichment.assessment_criteria && enrichment.assessment_criteria.length > 0 ? `
        <div style="margin-bottom: 32px;">
          <h3 style="font-size: 16px; font-weight: 600; color: #e0e0e0; margin-bottom: 12px;">
          ...
          <div style="background: #252525; padding: 16px; border-radius: 8px;">
            ...
            ${enrichment.assessment_criteria.map(criterion => `
...
function attachExerciseHandlers() {
  ...
  document.querySelectorAll('.btn-check-solution').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      await checkExerciseSolution(curriculumId, sectionId, exerciseId, editorId, outputId, feedbackId);
    });
  });
 ...
 */
async function checkExerciseSolution(curriculumId, sectionId, exerciseId, editorId, outputId, feedbackId) {
  const editor = document.getElementById(editorId);
  const outputDiv = document.getElementById(outputId);
  const outputContent = outputDiv.querySelector('.output-content');
...
function showCompleteSectionDialog(curriculumId, sectionId, sectionTitle) {
  ...
  currentSectionForCompletion = { curriculumId, sectionId, title: sectionTitle };
  ...
  document.querySelectorAll('.mastery-level').forEach(el => el.classList.remove('selected'));
...
function selectMasteryLevel(level) {
  ...
  document.querySelector(`.mastery-level[data-level="${level}"]`).classList.add('selected');
}
...
async function handleConfirmCompleteSection() {
  ...
  const struggles = document.getElementById('complete-section-struggles').value.trim();
  const breakthroughs = document.getElementById('complete-section-breakthroughs').value.trim();
...
async function loadCurriculumTemplates() {
  try {
    const response = await fetch('http://127.0.0.1:11436/polly/curricula/templates/list');
    if (!response.ok) {
...
function createWeekElement(week, subsections) {
  ...
  // Add subsections
  const subsectionsContainer = weekDiv.querySelector('.curriculum-week-subsections');
...
function createSubsectionElement(subsection) {
  ...
  const conceptsHtml = subsection.concepts && subsection.concepts.length > 0
...
function saveSectionEdits(sectionId, form) {
  ...
  const conceptsStr = form.querySelector('[data-field="concepts"]').value.trim();
  const estimatedTime = form.querySelector('[data-field="estimated_time"]').value.trim();
  
  // Update section
  section.title = title;
  section.description = description;
  section.concepts = conceptsStr ? conceptsStr.split(',').map(c => c.trim()) : [];
...
async function regenerateCurriculum() {
  ...
  const goal = document.getElementById('curriculum-goal-input')?.value || currentCurriculumData.goal || 'the curriculum';
  const selectedTemplateId = document.getElementById('curriculum-template-select')?.value || currentCurriculumData.template_id;
  ...
  let message;
  if (selectedTemplateId !== currentCurriculumData.template_id_original) {
    const templateName = document.getElementById('curriculum-template-select')
      ?.options[document.getElementById('curriculum-template-select')?.selectedIndex]
      ?.textContent || '';
  ...
  try {
    ...
    if (result.success) {
      ...
      if (metadata) {
        ...
        const tokensStr = metadata.tokens_in && metadata.tokens_out 
...
const loadDashboardData = withErrorBoundary(async function() {
  ...
  const statsResult = await safeFetch('http://127.0.0.1:11436/polly/stats', {}, silent);
  ...
  if (statsResult.ok && statsResult.data) {
    ...
    // Support both 'notes' (new native system) and 'obsidian' (legacy)
    const notesStats = data.rag?.notes || data.rag?.obsidian || { count: 0, files: 0 };
...
const loadKnowledgeData = withErrorBoundary(async function() {
  ...
  const notesSourceResult = await safeFetch('http://127.0.0.1:11436/polly/notes/source', {}, true);
  if (notesSourceResult.ok && notesSourceResult.data) {
    ...
    console.log('[Knowledge] Could not fetch notes path:', notesSourceResult.error);
  }
  ...
  console.log('[Knowledge] Stats fetch result:', { ok: statsResult.ok, hasData: !!statsResult.data });
  
  if (statsResult.ok && statsResult.data) {
    const data = statsResult.data;
    
    console.log('[Knowledge] Raw stats from server:', JSON.stringify(data.rag, null, 2));
    ...
    console.log('[Knowledge] Updating UI with stats:', { notesCount, notesFiles, codebaseCount, codebaseFiles });
...
async function resetPatterns() {
  ...
  try {
    const response = await fetch('http://127.0.0.1:11436/polly/patterns/reset', {
      method: 'POST'
    });
...
async function loadRoutingSettings() {
  try {
    ...
    updateRoutingSlider('routing-min-top-score', thresholds.min_top_score || 0.75);
    updateRoutingSlider('routing-min-quality-results', thresholds.min_high_quality_results || 2);
    updateRoutingSlider('routing-min-context-chars', thresholds.min_context_chars || 500);
    updateRoutingSlider('routing-exceptional-score', thresholds.exceptional_score || 0.85);
    updateRoutingSlider('routing-exceptional-min-results', thresholds.exceptional_min_results || 3);
...
async function loadRoutingStats() {
  try {
    ...
    if (statAvgLocalScore) statAvgLocalScore.textContent = data.avg_local_score ? data.avg_local_score.toFixed(2) : 'N/A';
    if (statCostSaved) statCostSaved.textContent = (data.cost_saved_today && typeof data.cost_saved_today === 'number') ? `$${data.cost_saved_today.toFixed(2)}` : '$0.00';
...
function initRoutingSettings() {
  ...
  if (aggressiveBtn) {
    aggressiveBtn.addEventListener('click', () => applyRoutingPreset('aggressive'));
  }
  ...
  const conservativeBtn = document.getElementById('routing-preset-conservative');
  if (conservativeBtn) {
    conservativeBtn.addEventListener('click', () => applyRoutingPreset('conservative'));
  }
}
...
async function saveRoutingSettings() {
  try {
    const settings = {
      enabled: true,
      thresholds: {
        min_top_score: parseFloat(document.getElementById('routing-min-top-score').value),
        min_high_quality_results: parseInt(document.getElementById('routing-min-quality-results').value),
        min_context_chars: parseInt(document.getElementById('routing-min-context-chars').value),
        exceptional_score: parseFloat(document.getElementById('routing-exceptional-score').value),
        exceptional_min_results: parseInt(document.getElementById('routing-exceptional-min-results').value)
      }
    };
...
function applyRoutingPreset(preset) {
  ...
  updateRoutingSlider('routing-min-quality-results', values.min_high_quality_results);
  ...
  updateRoutingSlider('routing-exceptional-min-results', values.exceptional_min_results);
...
async function restoreIntegrationStatus() {
  ...
  try {
    ...
    if (context7ApiKey && context7ApiKey.success && context7ApiKey.password) {
      ...
      // Reconnect to backend
      try {
        const backendResult = await fetch('http://127.0.0.1:11436/polly/integrations/connect', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
...
async function syncCalendar(daysAhead = 7) {
  ...
  try {
    const response = await fetch('http://127.0.0.1:11436/polly/integrations/sync', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
function initIntegrationPlaceholders() {
  ...
  if (context7SubmitBtn) {
    context7SubmitBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const response = await fetch('http://127.0.0.1:11436/polly/integrations/connect', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            integration: 'context7',
  ...
  if (obsidianSaveConfigBtn) {
    obsidianSaveConfigBtn.addEventListener('click', async () => {
      const pathInput = document.getElementById('obsidian-vault-path-input');
      const syncEnabledCheckbox = document.getElementById('obsidian-sync-enabled');
      ...
      try {
        ...
        await window.polly.updateConfig('notes.obsidian.sync_enabled', syncEnabled);
        ...
        if (syncEnabled) {
          ...
          if (result.success) {
            ...
            alert('Obsidian sync enabled! New notes will be copied to your vault.');
          } else {
            console.error('Obsidian connection failed:', result);
  ...
  if (obsidianTestCreateBtn) {
    obsidianTestCreateBtn.addEventListener('click', async () => {
      ...
      try {
        // Step 1: Get preview
        const previewResponse = await fetch('http://127.0.0.1:11436/polly/obsidian/create-note', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            title: title,
        ...
        const createResponse = await fetch('http://127.0.0.1:11436/polly/obsidian/create-note', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            confirmed: true,
...
async function suggestFolder() {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:11436/polly/obsidian/suggest-folder?title=${encodeURIComponent(title)}&content=${encodeURIComponent(content)}`);
...
async function suggestTags() {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:11436/polly/obsidian/suggest-tags?title=${encodeURIComponent(title)}&content=${encodeURIComponent(content)}`);
...
async function saveConversationToObsidian() {
  ...
  try {
    const messages = getCurrentConversationMessages();
    const response = await fetch('http://127.0.0.1:11436/polly/obsidian/from-conversation', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        messages: messages,
...
async function updateRelatedNotes() {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:11436/polly/obsidian/find-related?content=${encodeURIComponent(content)}&limit=8`);
...
const MENTAL_MODEL_TEMPLATES = {
  ...
  creative: {
    ...
    principles: ["Diverge before converging", "Embrace constraints", "Iterate rapidly"],
  ...
  productivity: {
    ...
    principles: ["Prioritize ruthlessly", "Time-box work", "Minimize context switching"],
  ...
  systems: {
    ...
    principles: ["Everything is connected", "Feedback loops matter", "Emergence is key"],
    prompt_injection: "Consider second-order effects and systemic implications.",
  ...
  communication: {
    ...
    principles: ["Clarity over cleverness", "Context is king", "Listen actively"],
...
async function loadMentalModels() {
  ...
  try {
    // Show loading state
    listContainer.innerHTML = '<div class="mental-models-loading">Loading mental models...</div>';
    ...
    if (mentalModels.length === 0) {
      ...
      mentalModels.forEach(model => {
        // Toggle
        const toggleEl = document.getElementById(`mental-model-toggle-${model.id}`);
        if (toggleEl) {
        ...
        // Delete
        const deleteBtn = document.getElementById(`mental-model-delete-${model.id}`);
        if (deleteBtn) {
...
function renderMentalModelCard(model) {
  ...
  if (model.active_on_pages && model.active_on_pages.length > 0) {
    ...
    if (model.active_on_pages.length > 3) {
      tags.push(`<span class="mental-model-tag page">+${model.active_on_pages.length - 3}</span>`);
    }
  }
...
function populateModalFromModel(model) {
  ...
  document.getElementById('mental-model-principles').value = model.principles.join('\n');
  document.getElementById('mental-model-prompt').value = model.prompt_injection;
  document.getElementById('mental-model-keywords').value = (model.keywords || []).join(', ');
  ...
  document.querySelectorAll('.page-checkbox').forEach(cb => {
    cb.checked = model.active_on_pages && model.active_on_pages.includes(cb.value);
  });
  
  document.querySelectorAll('.persona-checkbox').forEach(cb => {
    cb.checked = model.active_for_personas && model.active_for_personas.includes(cb.value);
  });
  
  document.querySelectorAll('.mode-checkbox').forEach(cb => {
    cb.checked = model.active_for_modes && model.active_for_modes.includes(cb.value);
  });
}
...
function applyMentalModelTemplate(templateId) {
  ...
  document.getElementById('mental-model-description').value = template.description;
  document.getElementById('mental-model-principles').value = template.principles.join('\n');
  document.getElementById('mental-model-prompt').value = template.prompt_injection;
  document.getElementById('mental-model-keywords').value = template.keywords.join(', ');
  ...
  document.querySelectorAll('.page-checkbox').forEach(cb => {
    cb.checked = template.active_on_pages && template.active_on_pages.includes(cb.value);
  });
...
async function saveMentalModel() {
  try {
    ...
    const description = document.getElementById('mental-model-description').value.trim();
    const principlesText = document.getElementById('mental-model-principles').value.trim();
    const promptInjection = document.getElementById('mental-model-prompt').value.trim();
    const keywordsText = document.getElementById('mental-model-keywords').value.trim();
    ...
    // Gather checkboxes
    const applies_to = Array.from(document.querySelectorAll('.domain-checkbox:checked')).map(cb => cb.value);
    const active_on_pages = Array.from(document.querySelectorAll('.page-checkbox:checked')).map(cb => cb.value);
    const active_for_personas = Array.from(document.querySelectorAll('.persona-checkbox:checked')).map(cb => cb.value);
    const active_for_modes = Array.from(document.querySelectorAll('.mode-checkbox:checked')).map(cb => cb.value);
...
async function toggleMentalModel(modelId) {
  try {
    const response = await fetch(`http://127.0.0.1:11436/polly/mental-models/${modelId}/toggle`, {
      method: 'POST'
    });
...
async function deleteMentalModel(modelId) {
  ...
  if (!confirm(`Delete mental model "${model.name}"?\n\nThis cannot be undone.`)) {
    return;
  }
  
  try {
    const response = await fetch(`http://127.0.0.1:11436/polly/mental-models/${modelId}`, {
      method: 'DELETE'
    });
...
async function refreshMentalModelsStats() {
  try {
    ...
    const activeResponse = await fetch('http://127.0.0.1:11436/polly/mental-models/active');
    ...
    console.log(`[Mental Models Stats] Total: ${totalCount}, Enabled: ${enabledCount}, Active: ${activeCount}`);
...
async function openMentalModelsOverrideModal(conversationId) {
  ...
  try {
    ...
    modelsList.innerHTML = allModels.map(model => {
      const isChecked = override && override.modelIds ? override.modelIds.includes(model.id) : model.enabled;
...
function closeMentalModelsOverrideModal() {
  document.getElementById('mental-models-override-modal').classList.add('hidden');
...
async function deleteDomain(domainId) {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:11436/polly/domains/${domainId}`, {
      method: 'DELETE'
    });
...
async function saveDomain() {
  ...
  try {
    ...
    if (editingDomainId) {
      // Update existing
      response = await fetch(`http://127.0.0.1:11436/polly/domains/${editingDomainId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
...
async function suggestDomainKeywords() {
  ...
  try {
    const response = await fetch('http://127.0.0.1:11436/polly/domains/suggest-keywords', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name,
...
async function toggleFolderNumbering(enabled) {
  try {
    const response = await fetch('http://127.0.0.1:11436/polly/domains/folder-numbering', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function saveDomainsOrder() {
  ...
  try {
    const response = await fetch('http://127.0.0.1:11436/polly/domains/reorder', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function loadDedupSettings() {
  try {
    const response = await fetch('http://127.0.0.1:11436/deduplication/settings');
    if (!response.ok) {
    ...
    const thresholdValue = document.getElementById('settings-dedup-threshold-value');
    const maxResultsSelect = document.getElementById('settings-dedup-max-results');
...
async function loadNotesSource() {
  try {
    ...
    if (sourceEl) {
      sourceEl.textContent = data.active_source === 'obsidian' ? 'Obsidian' : 'Native Polly';
    }
...
async function startMigration() {
  try {
    ...
    // Get options
    const options = {
      copy_attachments: document.getElementById('migration-copy-attachments')?.checked ?? true,
      preserve_structure: document.getElementById('migration-preserve-structure')?.checked ?? true,
      skip_canvas: document.getElementById('migration-skip-canvas')?.checked ?? true,
      switch_source: document.getElementById('migration-switch-source')?.checked ?? true
    };
    
    // Get Obsidian vault path from integrations
    const integrationsResponse = await fetch('http://127.0.0.1:11436/polly/integrations');
    ...
    if (!obsidianIntegration || !obsidianIntegration.vault_path) {
      showToast('Obsidian vault path not configured. Please configure Obsidian integration first.', 'error');
      return;
    }
    ...
    // Disable button
    btn.disabled = true;
    btn.innerHTML = '<i data-lucide="loader" style="width: 16px; height: 16px; margin-right: 6px; animation: spin 1s linear infinite;"></i><span>Starting...</span>';
    lucide.createIcons();
    
    // Start migration
    const response = await fetch('http://127.0.0.1:11436/polly/notes/migrate-from-obsidian', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i data-lucide="package" style="width: 16px; height: 16px; margin-right: 6px;"></i><span>Start Migration</span>';
      lucide.createIcons();
    }
  }
}
...
async function pollMigrationProgress(migrationId) {
  ...
  migrationPollInterval = setInterval(async () => {
    try {
      const response = await fetch(`http://127.0.0.1:11436/polly/notes/migrate/${migrationId}/status`);
      if (!response.ok) {
...
function showMigrationError(progress) {
  ...
  const errorMsg = progress.errors.length > 0 
...
async function switchToNativeSource(migrationId) {
  try {
    ...
    if (btn) {
      btn.disabled = true;
      btn.innerHTML = '<i data-lucide="loader" style="width: 16px; height: 16px; margin-right: 6px; animation: spin 1s linear infinite;"></i><span>Switching...</span>';
      lucide.createIcons();
    }
    
    const response = await fetch(`http://127.0.0.1:11436/polly/notes/migrate/${migrationId}/switch-source`, {
      method: 'POST'
    });
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i data-lucide="check" style="width: 16px; height: 16px; margin-right: 6px;"></i><span>Switch to Native Notes</span>';
      lucide.createIcons();
    }
  }
}
...
async function loadKnowledgeBasePath() {
  const result = await safeFetch('http://127.0.0.1:11436/polly/config/knowledge-base-path', {}, true);
  ...
  if (result.ok) {
    ...
    if (pathEl) {
      ...
      if (result.data.isDefault) {
        const suggestionsEl = document.getElementById('cloud-services-suggestions');
        if (suggestionsEl) {
...
async function loadCloudServices() {
  const result = await safeFetch('http://127.0.0.1:11436/polly/config/cloud-services', {}, true);
  ...
  if (result.ok && result.data.services) {
    ...
    if (availableServices.length > 0) {
      ...
      if (listEl) {
        listEl.innerHTML = availableServices.map(service => `
...
async function loadMigrationPreview() {
  const result = await safeFetch('http://127.0.0.1:11436/polly/config/migration-preview', {}, true);
  ...
  if (result.ok && result.data.stats) {
    ...
    if (previewEl) {
      ...
      document.getElementById('kb-migration-preview').classList.remove('hidden');
    }
  }
}
...
async function startKnowledgeBaseMigration() {
  ...
  try {
    ...
    // Update status
    document.getElementById('kb-migration-status-text').textContent = 'Preparing migration...';
    
    // Start migration
    const result = await safeFetch('http://127.0.0.1:11436/polly/config/migrate-knowledge-base', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (result.ok) {
      ...
      document.getElementById('kb-migration-progress-text').textContent = '100%';
      document.getElementById('kb-migration-status-text').textContent = 'Migration complete!';
      
      // Show completion step
      setTimeout(() => {
        document.getElementById('kb-migration-step-2').classList.add('hidden');
        document.getElementById('kb-migration-step-3').classList.remove('hidden');
...
async function startReindex() {
  try {
    ...
    const eventSource = new EventSource('http://127.0.0.1:11436/polly/notes/reindex');
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.status === 'started') {
          updateReindexProgress({ progress: 0, total: data.total, file: 'Starting...' });
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i data-lucide="refresh-cw" style="width: 16px; height: 16px; margin-right: 6px;"></i><span>Re-index All Notes</span>';
      lucide.createIcons();
    }
  }
}
...
async function loadCompressionSettings() {
  try {
    ...
    if (data.success && data.settings) {
      ...
      const thresholdInput = document.getElementById('compression-message-threshold');
...
function updateCompressionInputsState() {
  const enabled = document.getElementById('compression-enabled')?.checked || false;
...
async function saveCompressionSettings() {
  try {
    const enabled = document.getElementById('compression-enabled')?.checked || false;
    const threshold = parseInt(document.getElementById('compression-message-threshold')?.value || '20');
...
async function resetCompressionSettings() {
  ...
  try {
    ...
    showToast('Failed to reset compression settings: ' + error.message, 'error');
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const overrideCloseBtn = document.getElementById('close-mental-models-override-modal');
  if (overrideCloseBtn) {
    overrideCloseBtn.addEventListener('click', closeMentalModelsOverrideModal);
  }
  
  const overrideCancelBtn = document.getElementById('cancel-mental-models-override-modal');
  if (overrideCancelBtn) {
    overrideCancelBtn.addEventListener('click', closeMentalModelsOverrideModal);
  }
  
  const overrideSaveBtn = document.getElementById('save-mental-models-override-modal');
  if (overrideSaveBtn) {
...
function syncFloatingChatControls() {
  ...
  const floatingPersonaSelect = document.getElementById('floating-persona-select');
  const floatingModeSelect = document.getElementById('floating-mode-select');
  const floatingOrchestratorToggle = document.getElementById('floating-orchestrator-toggle');
  ...
  if (floatingPersonaSelect && personaSelect) {
    ...
    floatingPersonaSelect.addEventListener('change', () => {
      ...
      if (floatingPersonaSelect.value) {
        ...
        floatingModeSelect.disabled = true;
        floatingModeSelect.innerHTML = '<option value="">Select persona first</option>';
      }
    });
  ...
  if (floatingOrchestratorToggle && orchestratorToggle) {
    ...
    orchestratorToggle.addEventListener('click', () => {
      const isNowActive = orchestratorToggle.getAttribute('aria-checked') === 'true';
...
function setupFloatingChatListeners() {
  ...
  if (floatingTextarea) {
    floatingTextarea.addEventListener('input', () => {
      ...
      floatingTextarea.style.height = Math.min(floatingTextarea.scrollHeight, 180) + 'px';
    });
  ...
  setTimeout(() => {
    ...
    if (expandBtn) {
      expandBtn.onclick = function(e) {
        ...
        console.log('[Floating Chat] Current conversation ID:', currentConversationId);
        if (currentConversationId) {
          openChatOverlay(currentConversationId);
        } else {
          console.log('[Floating Chat] No conversation ID, cannot open overlay');
        }
        e.preventDefault();
        e.stopPropagation();
        return false;
...
async function sendFloatingMessageInternal(query) {
  ...
  try {
    ...
    console.log('[Floating Chat] Sending query with', conversationHistory.length, 'messages of history');
    
    // Route through persona if active
    if (activePersona) {
      const result = await sendToPersona(query, conversationHistory, loadingId, false); // false = use sidebar
      if (result) {
        return; // Persona handled the request
      }
    }
    ...
    if (result.success) {
      ...
      addMessageToUI('assistant', `<div class="error">Error: ${result.error}</div>`);
    }
  } catch (error) {
    console.error('[Floating Chat] Error sending message:', error);
    removeMessage(loadingId);
    addMessageToUI('assistant', `<div class="error">Error: ${error.message}</div>`);
  }
}
...
function detectPersonaIntent(query) {
  ...
  for (const [personaSlug, personaData] of Object.entries(PERSONA_INTENT_PATTERNS)) {
...
function canCurrentPersonaHandle(currentPersona, suggestedPersona, query) {
  ...
  if (currentPersona === 'professor' && suggestedPersona === 'architect') {
    ...
    const isLearningRequest = 
      /\b(learning|teaching|study|education|curriculum|lesson)\s+(plan|path|guide|roadmap)\b/i.test(query) ||
    ...
    if (isLearningRequest) {
      console.log('[Persona Intent] Professor curriculum mode can handle this learning plan');
      return true;
    }
  }
...
async function checkAndHandlePersonaSwitch(query, proceedCallback) {
  ...
  console.log('[Persona Intent] Current:', currentPersona, '| Suggested:', suggestedPersona);
...
async function openChatOverlay(conversationId) {
  ...
  console.log('[Floating Chat] Overlay opened for conversation:', conversationId);
}
...
function addMessageToOverlay(role, content) {
  ...
  const messageId = 'overlay-msg-' + Date.now() + '-' + Math.random().toString(36).substr(2, 9);
  const timestamp = new Date();
...
function setupSidebarResize() {
  ...
  setTimeout(() => {
    ...
    const conversationsSection = document.getElementById('chat-conversations-section');
...
async function executeCodeInSandbox(data) {
  ...
  if (data.editable) {
    html += '<button class="btn btn-sm btn-secondary" onclick="runCode(\''+execId+'\')">▶ Run</button>';
  }
  html += '</div>';
  html += '</div>';
...
async function runCodeExecution(execId, code, language, testCases) {
  const outputContent = document.querySelector(`#${execId}-output .output-content`);
  const testsDiv = document.getElementById(`${execId}-tests`);
  
  if (!outputContent) return;
  
  outputContent.innerHTML = '<div class="loading-spinner"></div><span>Running...</span>';
...
async function presentExercise(data) {
  ...
  if (data.type === 'code_challenge') {
    ...
    html += '<div class="exercise-feedback" id="'+exerciseId+'-feedback"></div>';
...
async function checkExerciseSolution(exerciseId) {
  ...
  feedback.innerHTML = '<div class="loading-spinner"></div><span>Checking...</span>';
  ...
  try {
    ...
    // Display feedback
    let html = '<div class="feedback-header ' + (allPassed ? 'feedback-success' : 'feedback-error') + '">';
    ...
    if (allPassed) {
      html += '<div class="feedback-success-message">Great job! You solved it correctly.</div>';
...
function updateTopicsUI(topics) {
  ...
  if (topics.length === 0) {
    topicsContent.innerHTML = '<p class="no-topics">No topics yet. Start learning!</p>';
    return;
  }
...
function showLearningNoteModal(data) {
  ...
  if (!modal) {
    ...
    // Setup event listeners
    document.getElementById('learning-note-close').addEventListener('click', () => {
      modal.classList.remove('show');
      modal.classList.add('hidden');
    });
    
    document.getElementById('learning-note-cancel').addEventListener('click', () => {
      modal.classList.remove('show');
      modal.classList.add('hidden');
    });
    
    document.getElementById('learning-note-create').addEventListener('click', async () => {
    ...
    document.getElementById('learning-note-preview-domain').textContent = domain;
