...
const TemplateGallery = {
  ...
  _attachListeners() {
    ...
    // Close button
    container.querySelector('.template-gallery-close').addEventListener('click', () => {
      this.hide();
    });
    
    // Close on overlay click
    container.querySelector('.template-gallery-overlay').addEventListener('click', () => {
      this.hide();
    });
    
    // Prevent modal close on modal content click
    container.querySelector('.template-gallery-modal').addEventListener('click', (e) => {
      e.stopPropagation();
    });
  },
  ...
  show(suggestedFilename = null, onSelect = null) {
    console.log('[TemplateGallery] Showing gallery, suggested:', suggestedFilename);
  ...
  _renderTemplates() {
    ...
    if (!this.templates || this.templates.length === 0) {
      grid.innerHTML = '<p class="template-gallery-empty">No templates available</p>';
      return;
    }
  ...
  _showPreview(template) {
    ...
    content.querySelector('.template-preview-description').textContent = template.description;
    content.querySelector('.template-preview-category').textContent = template.category;
    ...
    const variablesHtml = template.variables.slice(0, 8).map(v => 
      `<li><code>{{${v}}}</code></li>`
    ).join('');
    content.querySelector('.template-preview-variables').innerHTML = variablesHtml;
    if (template.variables.length > 8) {
      content.querySelector('.template-preview-variables').innerHTML += `<li>...and ${template.variables.length - 8} more</li>`;
    }
  },
  ...
  _selectTemplate(template) {
    ...
    console.log('[TemplateGallery] onSelectCallback exists:', !!this.onSelectCallback);
    ...
    if (callback) {
      console.log('[TemplateGallery] Calling onSelectCallback with:', template.filename);
      try {
