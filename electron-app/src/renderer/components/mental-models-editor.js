...
const MentalModelsEditor = {
  ...
  options: {
    ...
    pages: ['code', 'projects', 'learning', 'notes', 'patterns', 'dashboard', 'mail', 'calendar'],
  ...
  async init() {
    ...
    console.log('[MentalModelsEditor] Initialized with', this.models.length, 'models');
  },
  ...
  renderListView() {
    ...
    const tiers = {
      'Core Philosophy': ['infinite_games', 'instruments_over_tracks', 'constraint_as_meaning'],
      'Learning & Thinking': ['pedagogy_of_liberation', 'reverse_engineering', 'socratic_method'],
      'Systems & Technical': ['systems_thinking', 'first_principles', 'design_thinking'],
      'Communication': ['inbox_zero', 'async_first', 'time_blocking'],
      'Aesthetic & Craft': ['hyper_minimalism', 'readability_ratchet', 'local_variables', 'instrument_standard', 'sustainability_lens', 'signal_to_noise', 'generous_interface'],
      'Decision-Making': ['constraint_based_filter', 'compound_question', 'delegation_test', 'reverse_engineering_frame'],
      'Workflow & Process': ['continuation_probe', 'context_capture', 'good_enough_threshold', 'friction_audit', 'two_week_test'],
      'Architect-Specific': ['map_before_territory', 'three_altitudes', 'anchor_document'],
    ...
    let html = `
      ...
      <div class="mental-models-active-indicator">
        ...
        ${this.activeModels.length > 0 
          ? this.activeModels.map(m => `<span class="mm-badge mm-badge-active">${m.name}</span>`).join('')
    ...
    const customModels = this.models.filter(m => 
      !Object.values(tiers).flat().includes(m.id)
    );
  ...
  renderEditView(modelId) {
    const model = this.models.find(m => m.id === modelId) || { ...this.newModelTemplate, id: null };
      ...
      <div class="mental-models-detail-body">
          ...
          <div class="mm-detail-grid">
            <div>
              ...
              ${model.applies_to.length > 0 
                ? model.applies_to.map(d => `<span class="mm-badge">${d}</span>`).join(' ')
                : '<span class="mm-text-muted">None</span>'}
            </div>
            <div>
              <strong>Pages:</strong>
              ${model.active_on_pages.length > 0 
                ? model.active_on_pages.map(p => `<span class="mm-badge">${p}</span>`).join(' ')
                : '<span class="mm-text-muted">None</span>'}
            </div>
            <div>
              <strong>Personas:</strong>
              ${model.active_for_personas.length > 0 
                ? model.active_for_personas.map(p => `<span class="mm-badge">${p}</span>`).join(' ')
                : '<span class="mm-text-muted">None</span>'}
            </div>
            <div>
              <strong>Modes:</strong>
              ${model.active_for_modes.length > 0 
                ? model.active_for_modes.map(m => `<span class="mm-badge">${m}</span>`).join(' ')
                : '<span class="mm-text-muted">None</span>'}
            </div>
          </div>
        </div>
        ...
        <div class="mm-detail-section">
          ...
          ${model.keywords.length > 0 
            ? model.keywords.map(k => `<span class="mm-badge mm-badge-keyword">${k}</span>`).join(' ')
            : '<span class="mm-text-muted">None</span>'}
        </div>
  ...
  async saveModel(event) {
    ...
    const formData = {
      ...
      principles: document.getElementById('mm-edit-principles').value.split('\n').filter(p => p.trim()),
      prompt_injection: document.getElementById('mm-edit-prompt').value,
      applies_to: Array.from(document.querySelectorAll('.mm-checkbox-group input[value]:checked'))
        .filter(cb => this.options.domains.includes(cb.value))
        .map(cb => cb.value),
      active_on_pages: Array.from(document.querySelectorAll('.mm-checkbox-group input[value]:checked'))
        .filter(cb => this.options.pages.includes(cb.value))
        .map(cb => cb.value),
      active_for_personas: Array.from(document.querySelectorAll('.mm-checkbox-group input[value]:checked'))
        .filter(cb => this.options.personas.includes(cb.value))
        .map(cb => cb.value),
      active_for_modes: Array.from(document.querySelectorAll('.mm-checkbox-group input[value]:checked'))
        .filter(cb => this.options.modes.includes(cb.value))
        .map(cb => cb.value),
      keywords: document.getElementById('mm-edit-keywords').value
        .split(',')
        .map(k => k.trim())
        .filter(k => k),
    ...
    try {
      ...
      console.log(`[MentalModelsEditor] ${isNew ? 'Created' : 'Updated'} model:`, formData.name);
      ...
      this._showToast(`Model ${isNew ? 'created' : 'updated'} successfully!`, 'success');
  ...
  async toggleModel(modelId) {
    try {
      ...
      const data = await response.json();
      console.log(`[MentalModelsEditor] Toggled model ${modelId}:`, data.enabled);
      
      // Reload and refresh view
      await this.loadModels();
      this.renderListView();
      
      this._showToast(`Model ${data.enabled ? 'enabled' : 'disabled'}`, 'success');
  ...
  async deleteModel(modelId) {
    if (!confirm('Are you sure you want to delete this mental model? This cannot be undone.')) {
      return;
    }
