...
class ConversationManager {
    ...
    runMigrations() {
        try {
            ...
            const hasPageContext = tableInfo.some(col => col.name === 'page_context');
            ...
            if (hasEmojiIcons) {
                ...
                const updateStmt = this.db.prepare('UPDATE categories SET icon = ? WHERE id = ?');
                
                for (const [id, icon] of Object.entries(iconMapping)) {
                    updateStmt.run(icon, id);
                }
                
                console.log('Migration complete: Category icons updated to Lucide names');
            }
        } catch (error) {
    ...
    getConversation(id, options = {}) {
        ...
        // Get messages
        const messages = this.getMessages(id, options.messageLimit, options.messageOffset);
    ...
    async triggerPatternLearning(conversationId) {
        try {
            ...
            const response = await fetch('http://localhost:11436/polly/patterns/learn-from-conversation', {
                method: 'POST',
                headers: {
            ...
            console.log(`Pattern learning: ${result.patterns_learned} patterns from conversation ${conversationId}`);
        } catch (error) {
     ...
     */
    toggleStar(id) {
        const current = this.db.prepare('SELECT is_starred FROM conversations WHERE id = ?').get(id);
     ...
     */
    togglePin(id) {
        const current = this.db.prepare('SELECT is_pinned FROM conversations WHERE id = ?').get(id);
        if (!current) return null;
    ...
    needsAutoTitle(id) {
        ...
        // - Still has default title
        return conv.message_count >= this.autoTitleThreshold 
            && !conv.auto_titled 
