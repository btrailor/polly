...
| `MCP_ADAPTER.md` | 2 | 💡 Idea | MCP transport adapter for OpenClaw skill runner. Wraps any MCP server as a native skill — `sandbox-exec` isolation, manifest-driven sandbox profile, stdio-only IPC, process lifecycle tied to skill session. **Depends on: `SKILLS_MARKETPLACE.md`**. Refs: https://github.com/modelcontextprotocol/servers, https://github.com/punkpeye/awesome-mcp-servers |
