...
- **Phase 24e Complete (Feb 27, 2026):** Advanced Nexus features. `core/nexus/prompt_agent.py` (PromptAgent — user-defined LLM agent from a system prompt; full Executable Protocol). `SwarmStorage.prompt_agents` table (CRUD for persisted prompt agents). Critical bug fix: `polly.py:_init_nexus` now properly wires `TemplateRegistry`, `NexusContextBroker`, and `pattern_engine` to `NexusCoordinator` — previously `/swarms/workflow` returned 503 and template list was always empty. `NexusCoordinator` gains `pattern_engine` param + `learn()` call after successful `execute_workflow()` (PatternType.WORKFLOW). 2 new server endpoints: `GET /swarms/capabilities` (flat list of all persona + prompt-agent capabilities) and `POST /agents` (create PromptAgent at runtime). Usage count fix in `GET /swarms/templates` response (merges `_usage_count` from storage). Frontend: "New Template" + "New Agent" header buttons; template-builder modal (step builder, domain chips, merge strategy, output step select); prompt-agent-builder modal; `swarms.css` additions. 32 new tests + 360 total nexus tests all passing.
...
### Knowledge Graph Advanced (Phase 12b) ✅ Complete (March 2026)

- **Status:** All 3 waves complete. 26/26 tasks done. 91 tests passing.
- **Wave 1 (Graph-Aware Retrieval):** GraphRetriever as ContextContributor (priority 45), authority scoring as 3rd RRF signal in HybridSearcher, path-finding API endpoint, config entries. 31 tests.
- **Wave 2 (Quality Controls):** Isolated entity/note detection (`get_isolated_entities`, `get_isolated_notes`), garden maintenance digest endpoint, batch operations endpoint, authority recomputation scheduling. Frontend: isolation badges, garden digest panel. 25 tests.
- **Wave 3 (Graph Intelligence):** Community detection (label propagation), PageRank + betweenness centrality, upgraded authority formula (0.4×pagerank + 0.3×log(mentions) + 0.2×betweenness + 0.1×recency), edge confidence scoring (4-factor: co-occurrence, semantic similarity, temporal proximity, structural proximity), "Why this connection?" API, communities API. Frontend: community cluster coloring toggle, path highlighting, edge confidence modal. 35 tests.
- **Key files:** `core/entities/retriever.py`, `core/entities/intelligence.py`, `core/entities/store.py` (extended), `core/hybrid_search.py` (extended), `interfaces/server.py` (7 new endpoints), `electron-app/src/renderer/app.js` (graph features)
- **OpenSpec:** [openspec/changes/knowledge-graph-advanced/](../../changes/knowledge-graph-advanced/)
